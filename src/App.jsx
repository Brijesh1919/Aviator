...
import MinesGame from './components/Mines/MinesGame.jsx';
import { useGame } from './context/GameContext.jsx';

export default function App() {
  const { balance } = useGame();
  const [view, setView] = useState('dashboard'); // 'dashboard' | 'aviator' | 'mines'

  const handlePlay = (target = 'aviator') => setView(target);
  const handleBack = () => setView('dashboard');

  return (
    <div className="flex flex-col h-full select-none">
      {view !== 'mines' && <TopBar balance={balance} />}
